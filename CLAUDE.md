...
**Styling is one global stylesheet** (`src/style.css`) using CSS custom properties, no CSS modules or utility framework. The key mechanism to understand before touching any page style: the persistent header/footer/mobile-drawer chrome uses one token set (`--panel`, `--panel-ink`, `--panel-muted`, deep ink navy), while every routed page is wrapped in a `.page` element that *re-scopes* the shared tokens (`--bg`, `--fg`, `--muted`, `--faint`, `--hairline`, `--bg-raised`) to a second, flat powder-blue palette (`--menu-bg`, `--menu-ink`, `--menu-muted`). Because components like `.button`, `.textLink`, and `.photo` are written against the generic tokens rather than hardcoded colors, they automatically pick up the right palette depending on whether they render inside `.page` or not — new components should follow the same pattern (style against `var(--fg)`/`var(--muted)`/etc., not literal hex values) rather than adding page-specific overrides.

Two font roles, both loaded via Google Fonts in `index.html`: `--font-display` (Bricolage Grotesque) for headings, nav, and buttons; the base `font-family` (Zen Kaku Gothic New) for body text.
